...
{
    return Init("127.0.0.1", 3306, "root", "root", "QQ_Server");//ini从配置文件获取
}
...
bool MysqlConnection::Init(const char * szHost, int nPort, const char * szUser, const char * szPasswd, const char * szDb)
{
	...
	if (mysql_ == NULL)
	{
		LOG_ERROR <<"init mysql failed "<< this->GetErrInfo() <<", "<< errno;
		return false;
	}
	char cAuto = 1;
	...
	{
		LOG_ERROR <<"connect mysql failed "<< this->GetErrInfo() <<", "<< errno;
		return false;
	}
